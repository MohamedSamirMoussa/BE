...
export const getConnectionWithServer = async (
  ...
  try {
    ...
    const leaderboard: ILeaderboardUser[] = [];
    ...
    for (const username of usernames) {
      try {
        ...
        const player = await upsertPlayer(username, isOnline, serverName, playTimeInSec);
        leaderboard.push(player);
        
      } catch (playerError) {
        console.error(`Error processing player ${username}:`, playerError);
        continue;
      }
    }
